...
使用List<char[]>来表示某种可能在返回结果List<List<String>>时需要进行一些转换

根据皇后的攻击返回，在放置一个皇后以后在棋盘上的八个访问一直延申直至底部都无法再放置皇后每放置一个皇后就需要更新棋盘，标记哪些位置还可以放置哪些不行

使用回溯思想尝试着在棋盘的每一个地方放置棋子，直到无法放置则回溯到上一层，换一个位置放不再从头开始
